...
gss_person <- read_sql_table(paste0("select *, 1 as counter from ",schemaname,".of_gss_ind_variables_sh3_trt pers 
                             left join ",schemaname,
                             ".of_gss_calibrated_weights_sh3 wt
...
gss_person_final <- gss_person %>%
  ...
  mutate(pub_trpt_safety_ind = ifelse(get("pub_trpt_safety_ind") == 1, "Safe","Unsafe/Unknown")
         ...
         ,acc_sup_ind = ifelse(get("accom_sup_1yr_b4_intvw") > 0, "Yes","No/Unknown")
         ...
         ,wellbeingind = ifelse(get("gss_id_collection_code") %in% c("GSS2014","GSS2016"), ifelse(get("gss_pq_material_wellbeing_code") <= 9, "Low", "High"), 
         ...
         ,mwi_bottom_20 = ifelse(get("gss_id_collection_code") %in% c("GSS2014","GSS2016") | is.na(get("gss_pq_ELSIDV1")), "Unknown", 
                                 ifelse(get("gss_pq_ELSIDV1") <= 31*0.2, "Yes", "No"))
         ,wellbeingind_081012 = ifelse(get("gss_id_collection_code") %in% c("GSS2014","GSS2016") | is.na(get("gss_pq_ELSIDV1")), "Unknown",
                                       ifelse(get("gss_pq_ELSIDV1") <= 18, "Low", "High"))
  )
...
for(wave in levels(gss_person_final$gss_id_collection_code) ) {
  ...
  write.xlsx(as.data.frame(univariate_tbl), file = "../output/univariate_desc_stats1.xlsx", sheetName = wave, row.names = FALSE,
              append = TRUE)
  ...
  for(i in 1:length(gssvars)){
    print(i)
    bivariate_tbl_housing <- rbind(bivariate_tbl_housing, bivariate_descr(quo(housing_groups_pq), gssvars[[i]] ))
  }
  
  write.xlsx(as.data.frame(bivariate_tbl_housing), file = "../output/bivariate_housing_desc_stats_weighted1.xlsx", sheetName = wave, row.names = FALSE,
              append = TRUE)
...
univar_agg <- univariate_list$GSS2016 %>% dplyr::select(var, var_name, wtmean, wtmean_se, wtmean_low, wtmean_upp, unwttotal)%>%
  ...
  full_join(univariate_list$GSS2014 %>% dplyr::select(var, var_name, wtmean, wtmean_se, wtmean_low, wtmean_upp, unwttotal)%>%
              dplyr::rename(wtmean14=wtmean,wtmean_se14=wtmean_se, wtmean_low14=wtmean_low, wtmean_up14=wtmean_upp,unwttotal14=unwttotal) , by = c("var", "var_name")) %>%
  full_join(univariate_list$GSS2012 %>% dplyr::select(var, var_name, wtmean, wtmean_se, wtmean_low, wtmean_upp, unwttotal)
            %>%
              dplyr::rename(wtmean12=wtmean,wtmean_se12=wtmean_se, wtmean_low12=wtmean_low, wtmean_up12=wtmean_upp,unwttotal12=unwttotal), by = c("var", "var_name")) %>%
  full_join(univariate_list$GSS2010 %>% dplyr::select(var, var_name, wtmean, wtmean_se, wtmean_low, wtmean_upp, unwttotal)
            %>%
              dplyr::rename(wtmean10=wtmean,wtmean_se10=wtmean_se, wtmean_low10=wtmean_low, wtmean_up10=wtmean_upp,unwttotal10=unwttotal), by = c("var", "var_name")) %>%
  full_join(univariate_list$GSS2008 %>% dplyr::select(var, var_name, wtmean, wtmean_se, wtmean_low, wtmean_upp, unwttotal)
            %>%
              dplyr::rename(wtmean08=wtmean,wtmean_se08=wtmean_se, wtmean_low08=wtmean_low, wtmean_up08=wtmean_upp,unwttotal08=unwttotal), by = c("var", "var_name")) %>%
...
write.xlsx(as.data.frame(univar_agg), file = "../output/univariate_agg1.xlsx", sheetName = "Data", row.names = FALSE)

# Calculate aggregate statistics for bivariate case
bivar_agg <- bivariate_housing_list$GSS2016 %>% dplyr::select(var, var_name, var2, var_name2, wtmean, wtmean_se, wtmean_low, wtmean_upp, unwttotal) %>%  
  dplyr::rename(wtmean16=wtmean,wtmean_se16=wtmean_se, wtmean_low16=wtmean_low, wtmean_up16=wtmean_upp,unwttotal16=unwttotal)%>%  
  full_join(bivariate_housing_list$GSS2014 %>% dplyr::select(var, var_name, var2, var_name2, wtmean, wtmean_se, wtmean_low, wtmean_upp, unwttotal)%>%
              dplyr::rename(wtmean14=wtmean,wtmean_se14=wtmean_se, wtmean_low14=wtmean_low, wtmean_up14=wtmean_upp,unwttotal14=unwttotal) , 
            by = c("var", "var_name", "var2", "var_name2")) %>%
...
write.xlsx(as.data.frame(bivar_agg), file = "../output/bivariate_agg1.xlsx", sheetName = "Data", row.names = FALSE)
...
overall_tbl <- gss_person %>% 
  ...
  summarise(unwttot = n()
            ,wttot = sum(gss_pq_person_FinalWgt))

write.xlsx(as.data.frame(overall_tbl), file = "../output/overall_desc_stats_unweighted1.xlsx", sheetName = "All_waves", row.names = FALSE, 
           append = TRUE)
