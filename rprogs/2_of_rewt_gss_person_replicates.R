...
dataset <- SIAtoolbox::read_sql_table(paste0("select *
                                      from IDI_Sandpit.",schemaname,".[of_gss_ind_variables_sh3_trt]"),
                                      connstr, string = TRUE)
...
person <- dataset %>%
  ...
  mutate(
    ...
    ,two_adult = factor(ifelse(adult_ct == 2, 1, 0))
    ,one_adult = factor(ifelse(adult_ct == 1, 1, 0))
   ...
   ,hh_admin_income_ind = factor(ifelse(total_equi_income_net_b4 <= 0,0 , 1))
  )
...
connstr <- paste0(connstr, "SERVER=SERVERNAME, PORTNUMBER")
