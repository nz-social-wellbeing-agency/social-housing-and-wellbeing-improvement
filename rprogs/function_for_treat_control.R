...
robust_glm_wrapper <- function(formula_matrix, dataset, dependent_var, suffix="") {
  
  ds <- filter_(dataset, paste0(dependent_var," != \"IGNORE\""))
  
  # Create the output files and the index page
  write.xlsx2(formula_matrix, file=paste0("../output/coeff_glmrob_",dependent_var,suffix, ".xlsx"), sheetName="Index");
  sink(paste0("../output/coeff_glmrob_",dependent_var,suffix,".csv"))
  ...
  for (i in 1:nrow(formula_matrix) ){
    ...
    print(summary(robust_glm))
    vif <- car::vif(mod = robust_glm)
    print(vif)
    # print(summary(vif))
    xlsx::write.xlsx(y$coefficients, file=paste0("../output/coeff_glmrob_",dependent_var,suffix, ".xlsx"), 
...
glm_wrapper <- function(formula_matrix, dataset, dependent_var, suffix="") {
  
  ds <- filter_(dataset, paste0(dependent_var," != \"IGNORE\""))
  
  
  # Create the output files and the index page
  write.xlsx(formula_matrix, file=paste0("../output/coeff_glm_",dependent_var,suffix, ".xlsx"), sheetName="Index");
  sink(paste0("../output/coeff_glm_",dependent_var,suffix,".csv"))
  ...
  for (i in 1:nrow(formula_matrix) ){
    ...
    y <- summary(glm)
    print(summary(glm))
    xlsx::write.xlsx(y$coefficients, file=paste0("../output/coeff_glm_",dependent_var,suffix, ".xlsx"), 
                    sheetName=str_sub(reg_formulas[i,1],1,str_locate(reg_formulas[i,1],"~")[1]-1), append=TRUE);
