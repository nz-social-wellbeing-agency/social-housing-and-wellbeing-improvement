...
# 28 Nov 2017 SA v1
# 01 Dec 2018 BV Adapted for Social Housing 3 (tabs, t-test and regressions and main group and sens only)
# ================================================================================================ #
...
dataset <- SIAtoolbox::read_sql_table(paste0("SELECT *
                                      FROM [IDI_Sandpit].",sandpitname,".[of_hnz_gss_population_wide_sh3]")
                                      ,connstr, string = TRUE)

################ Data re-labeling & preparations ################

# list of variable columns needed in dataset
column_names_used <- c("snz_sex_code", 
                       ...
                       "house_crowding_ind", "gss_pq_house_cold_code", "gss_pq_house_condition_code", "gss_pq_house_mold_code",
                       ...
                       "snz_spine_ind", "num_overseas_trips", "life_satisfaction_bin","gss_pq_highest_qual_dev",
                       
                       "num_address_changes", "adult_count",
                       "housing_groups", "life_satisfaction_ind", "gss_pq_dvage_code", "months_interview_to_entry",
                       
                       "gss_pq_phys_health_code", "gss_pq_ment_health_code", "gss_pq_HH_crowd_code",
                       "gss_pq_cult_identity_code", "gss_pq_lfs_dev", "gss_pq_material_wellbeing_code", "gss_pq_ELSIDV1",
                       "phys_health_sf12_score", "ment_health_sf12_score",
                       
                       # annual GSS income
                       "gss_hq_household_inc1_dev",
                       "hh_gss_income" ,
                       "hh_gss_income_median",
                       "hh_gss_equi_income_median_inf",
                       "hh_gss_equi_income_median_lw_inf",
                       "hh_gss_equi_income_median_up_inf",
                       
                       # annual admin income
                       "total_income_monthly_before_hld",
                       "total_equi_income_net_b4",
                       
                       "income_monthly_net_before_hld",
                       "hh_equi_income_net_b4",
...
dataset <- mutate(dataset, house_condition_issue = ifelse(gss_pq_house_condition_code == 1,'Y','N')) # no 2014 and 2016 
...
dataset <- mutate(dataset, housing_sat_ind = ifelse(housing_sat_ind == 1,'Y','N')) # no 2014 and 2016
# community outcomes
...
dataset[is.na(dataset$time_lonely_ind), "time_lonely_ind"] <- 0
dataset <- mutate(dataset, time_lonely_ind = ifelse(time_lonely_ind ==1,'Y','N'))
dataset[is.na(dataset$voting_ind), "voting_ind"] <- 0
dataset <- mutate(dataset, voting_ind = ifelse(voting_ind == 1,'Y','N')) # no 2014 only
dataset[is.na(dataset$cult_identity_ind), "cult_identity_ind"] <- 0
...
dataset <- mutate(dataset, no_access_natural_space = ifelse(no_access_natural_space == 1,'Y','N')) # no 2014 2016
dataset <- mutate(dataset, no_free_time = ifelse(no_free_time == 1,'Y','N')) # no 2014 and 2016
...
dataset <- mutate(dataset, wave = ifelse(gss_pq_collection_code == "GSS2008",1,
                                                 ifelse(gss_pq_collection_code == "GSS2010",2,
                                                        ifelse(gss_pq_collection_code == "GSS2012",3,
                                                               ifelse(gss_pq_collection_code == "GSS2014",4,5)))))

# Time relative to date housed
dataset <- mutate(dataset, time_to_housed = as.numeric(as.Date(dataset$interview_date) - as.Date(dataset$tenancy_entry_date))
                         , time_after_housed = ifelse(as.Date(dataset$interview_date) - as.Date(dataset$tenancy_entry_date)>0,
                                                      as.Date(dataset$interview_date) - as.Date(dataset$tenancy_entry_date), 0 )
                         , time_before_housed = ifelse(as.Date(dataset$tenancy_entry_date) - as.Date(dataset$interview_date)>0,
                                                       as.Date(dataset$tenancy_entry_date) - as.Date(dataset$interview_date),0 ) )
...
# Monthly acommodation supplement
dataset <- mutate(dataset, hh_accom_supp_month=accom_sup_1yr_b4_intvw_hld/12)



#Converting to numeric and mid point to the income band... 
#instead of doing the median here, we are using the hh_gss_income_median for the numeric value (SH3)
...
dataset[is.na(dataset$housing_groups), "housing_groups"] <- 'Other'
...
# impute missing sf12 to the average value
...
dataset <- mutate(dataset, sf12_ment_hth = cut(gss_pq_ment_health_code, c(-1,35,50,100),
                                              labels=c('low (0-35)','medium (36-50)','high (51+)')))
# NZ norms SF12
dataset <- mutate(dataset, phys_health_sf12_score = ifelse(phys_health_sf12_score == 777,NA,phys_health_sf12_score))
dataset <- mutate(dataset, ment_health_sf12_score = ifelse(ment_health_sf12_score == 777,NA,ment_health_sf12_score))
dataset <- mutate(dataset, nzsf12_phys_hth = cut(phys_health_sf12_score, c(-1,35,50,100),
                                               labels=c('low (0-35)','medium (36-50)','high (51+)')))
dataset[is.na(dataset$nzsf12_phys_hth), "nzsf12_phys_hth"] <- 'medium (36-50)'

dataset <- mutate(dataset, nzsf12_ment_hth = cut(ment_health_sf12_score, c(-1,35,50,100),
                                               labels=c('low (0-35)','medium (36-50)','high (51+)')))
dataset[is.na(dataset$nzsf12_ment_hth), "nzsf12_ment_hth"] <- 'medium (36-50)'
...
dataset <- mutate(dataset, material_wellbeing_ELSI = "lower 20%")
...
dataset[is.na(dataset$labour_force_status), "labour_force_status"] <- 'not in labour force'
#

#### TIDY UP ####

# house_condition and housing_satisfaction were not asked in the 2014 and 2016 GSS waves
# hence the 2014 and 2016 values are set equal to NA
dataset <- mutate(dataset, house_condition_issue = ifelse(gss_pq_collection_code %in% c("GSS2014","GSS2016"),NA,house_condition_issue))
dataset <- mutate(dataset, housing_sat_ind = ifelse(gss_pq_collection_code %in% c("GSS2014","GSS2016"),NA,housing_sat_ind))

# indicators missing for 2014 and/or 2016
dataset <- mutate(dataset, voting_ind = ifelse(gss_pq_collection_code %in% c("GSS2014"),NA,voting_ind))

dataset <- mutate(dataset, no_access_natural_space = ifelse(gss_pq_collection_code %in% c("GSS2014","GSS2016"),NA,no_access_natural_space))

dataset <- mutate(dataset, no_free_time = ifelse(gss_pq_collection_code %in% c("GSS2014","GSS2016"),NA,no_free_time))
...
columns_to_analyse <- list(
  ...
  c('hh_income', 'GSS Household income category (from NZGSS)','both'),
  
  c("income_monthly_net_before_hld", 'Admin Household income (Monthly average year before itw)','test'),
  c("hh_equi_income_net_b4", 'Admin Household equivalised income (Monthly average year before itw)','test'),
  c("hh_accom_supp_month", 'Admin Household accom supp (Monthly average year before itw)','test'),
  

  c('gss_pq_collection_code', 'NZGSS wave','both'),
  c('months_interview_to_entry', 'Months between interview and entry date','table'),
  c('time_to_housed', 'Days between entry date and interview','test'),
  ...
  c('nzsf12_phys_hth', 'Categorized NZSF12 physical health','table'),
  c('nzsf12_ment_hth', 'Categorized NZSF12 mental health','table'),
  c('phys_health_sf12_score', 'NZSF12 physical health','test'),
  c('ment_health_sf12_score', 'NZSF12 mental health','test'),
